...
static void nvme_process_sq_io(void *opaque, int index_poller)
{
    ...
    uint16_t status;
    hwaddr addr;
    NvmeCmd cmd;            // command 就是 64 字节一条指令
    NvmeRequest *req;          // 而 request 则包含更多信息，其中包括了 command
    int processed = 0;

    nvme_update_sq_tail(sq);       // 更新队尾，检查是否有新的 sqe
    while (!(nvme_sq_empty(sq))) {      // 只要队列不为空，就一直处理
        if (sq->phys_contig) {
            ...
            addr = nvme_discontig(sq->prp_list, sq->head, n->page_size,
                                  n->sqe_size);
            nvme_addr_read(n, addr, (void *)&cmd, sizeof(cmd));       // 从内存中读到了命令
        }
        nvme_inc_sq_head(sq);      // 刚取出队头 sqe ，将队头往后走一个

        // 接下来是根据取出的 cmd ，生成对应的 req 
    ...
    sq->completed += processed;          // 该 sq 队列的 sqe 完成数
}
...
static void *nvme_poller(void *arg)
{
    ...
    case 1:
        while (1) {
            ...
            NvmeSQueue *sq = n->sq[index];           // *sq 链表头指针 ,sq 队列是一个链表结构
